...
<select id="${status.expression?replace('[','')?replace(']','')}" name="${status.expression}" ${attributes}>
    ...
    <#elseif options?is_sequence>
        <#list options as option>
            <#assign value = option.id?string>
            <option value="${value?html}"<@checkSelected value/>>${option.value?html}</option>
        </#list>
    <#else>
 ...
 *    or CSS styles or size
-->
<#macro formSingleGroupSelect path options attributes="">
    <@bind path/>
<select id="${status.expression?replace('[','')?replace(']','')}" name="${status.expression}" ${attributes}>
    <#list options as value>
        <optgroup label="${value.value?html}">
            <#list value.leaf as value_leaf>
                <option value="${value_leaf.id}"<@checkSelected value_leaf.id?string />>${value_leaf.value}</option>
            </#list>
        </optgroup>
    </#list>
</select>
...
<#macro formRadioButtons path options separator attributes="">
    ...
    <#list options?keys as value>
        ...
        ${separator}<input type="radio" id="${id}" name="${status.expression}" value="${value?html}"<#if stringStatusValue == value> checked="checked"</#if> ${attributes}<@closeTag/>
        <span class="lbl"> ${options[value]?html}</span>
    </#list>
</#macro>
